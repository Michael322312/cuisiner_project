...
class CustomUserCreationForm(UserCreationForm):
    password1 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={
                "class": "form-control mb-3 w-75 rounded",
...
class CustomUserUpdateForm(UserChangeForm):
    class Meta:
        ...
        widgets = {
            ...
            "avatar": forms.FileInput(
                attrs={"class": "form-control mb-3 w-75 rounded"}
            ),
            "intro": forms.Textarea(
                attrs={
                    "class": "form-control mb-3 w-75 rounded",
...
class UserPreferenceCreateForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "fav_categories": forms.SelectMultiple(
                attrs={"class": "chosen-select"}
            ),
            "hate_categories": forms.SelectMultiple(
                attrs={"class": "chosen-select"}
            ),
            "fav_products": forms.SelectMultiple(
                attrs={"class": "chosen-select"}
            ),
            "hates_products": forms.SelectMultiple(
                attrs={"class": "chosen-select"}
            ),
